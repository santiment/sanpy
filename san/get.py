...
from san.query_constants import DEPRECATED_QUERIES, CUSTOM_QUERIES, NO_SLUG_QUERIES, QUERY_MAPPING
...
def __get(query, **kwargs):
    ...
    if query in QUERY_MAPPING.keys():
        ...
        gql_query = (
            "{"
            + san.sanbase_graphql.get_metric_timeseries_data(idx, query, **kwargs)
            + "}"
        )
