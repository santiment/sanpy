...
def transform_query_args(query, **kwargs):
    kwargs["from_date"] = (
        kwargs["from_date"] if "from_date" in kwargs else _default_from_date()
    )
    kwargs["to_date"] = kwargs["to_date"] if "to_date" in kwargs else _default_to_date()
    kwargs["interval"] = (
        kwargs["interval"] if "interval" in kwargs else _DEFAULT_INTERVAL
    )
    kwargs["social_volume_type"] = (
        kwargs["social_volume_type"]
        if "social_volume_type" in kwargs
        else _DEFAULT_SOCIAL_VOLUME_TYPE
    )
    kwargs["source"] = kwargs["source"] if "source" in kwargs else _DEFAULT_SOURCE
    kwargs["search_text"] = (
        kwargs["search_text"] if "search_text" in kwargs else _DEFAULT_SEARCH_TEXT
    )
    kwargs["aggregation"] = kwargs["aggregation"] if "aggregation" in kwargs else "null"
    kwargs["include_incomplete_data"] = (
        kwargs["include_incomplete_data"]
        if "include_incomplete_data" in kwargs
        else False
    )
    # transform python booleans to strings so it's properly interpolated in the query string
    kwargs["include_incomplete_data"] = (
        "true" if kwargs["include_incomplete_data"] else "false"
    )
    ...
    kwargs["transaction_type"] = (
        kwargs["transaction_type"] if "transaction_type" in kwargs else "ALL"
    )
