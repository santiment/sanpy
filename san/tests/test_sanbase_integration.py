...
METRICS_USING_ETHEREUM = ["gas_used", "mining_pools_distribution"]
...
def test_average_token_age_consumed_in_days():
    _test_ordinary_function("average_token_age_consumed_in_days")
...
def test_mining_pools_distribution():
    _test_ordinary_function("mining_pools_distribution", "ethereum")
...
def test_social_dominance():
    ...
    for item in sources:
        result = san.get(
            "social_dominance/" + params["project_slug"],
            from_date=params["from_date"],
            to_date=params["to_date"],
            interval=params["interval"],
            source=item,
...
def test_price_volume_difference():
    ...
    for item in currencies:
        result = san.get(
            "price_volume_difference/" + params["project_slug"],
            from_date=params["from_date"],
            to_date=params["to_date"],
            interval=params["interval"],
            currency=item,
...
def test_token_top_transactions():
    result = san.get(
        "token_top_transactions/" + params["project_slug"],
...
def test_social_volume():
    social_volume_types = ["TELEGRAM_CHATS_OVERVIEW", "TELEGRAM_DISCUSSION_OVERVIEW"]
    for item in social_volume_types:
        result = san.get(
            "social_volume/" + params["project_slug"],
            from_date=params["from_date"],
            to_date=params["to_date"],
            interval=params["interval"],
...
def test_available_metric_for_slug_since():
    assert (
        available_metric_for_slug_since("daily_active_addresses", "santiment")
