    ...
    * Init_asset
    ...
    def __sort_asset_types(self, assets_type, c_case, r_case, skip_a_case=False):
        if not skip_a_case and assets_type.lower() in ("a", "all"):
            ...
            return c_case()
        logging.error(
            f'Asset type {assets_type} is invalid.\
    ...
    def add(self, assets: dict, assets_type="common"):
        ...
        def _update_assets(assets, new_assets, granularity=self._granularity):
            ...
            for asset_name in new_assets:
                ...
                assert (
                    min(dates) >= self._start_dt
                ), f"Provided datetime ({min(dates)}) is smaller than expected ({self._start_dt}). [{asset_name}]"
                assert (
                    len(dates) % 2 == 0
                ), f"Unsupported datetime sequence for {asset_name}: odd amount of dates."
            ...
            return (
                assets.reset_index().drop_duplicates().set_index("index").sort_index()
            )
        ...
        def _test_asset_name(new_assets, assets):
            ...
            assets_names = set(
                assets["asset"].unique()
            )  # works faster than set(assets['asset']) when len(df) is big
            for asset_name in new_assets:
                assert (
                    asset_name not in assets_names
        ...
        if assets_type.lower() in ("r", "res", "reserve"):
            ...
            self.reserve_assets = _update_assets(
                assets=self.reserve_assets, new_assets=assets
            )
        elif assets_type.lower() in ("c", "com", "common"):
            _test_asset_name(new_assets=assets, assets=self.reserve_assets)
            self.common_assets = _update_assets(
                assets=self.common_assets, new_assets=assets
            )
        else:
            logging.error(
                f"Asset type {assets_type} is not valid. Asset type must be one of (common, reserve)"
            )
    ...
    def remove(self, assets: dict):
        ...
        def _remove_assets(
            assets_df, exclude_asset, exclude_dates, granularity=self._granularity
        ):
            dates = [str_to_ts(dt) for dt in exclude_dates]
            assert (
                len(dates) % 2 == 0
            ), f"Unsupported datetime sequence for {exclude_asset}: odd amount of dates."

            for i in range(int(len(dates) / 2)):
                exclude_dates = list(
                    pd.date_range(
                        start=dates[2 * i], end=dates[2 * i + 1], freq=granularity
                    )
                )
                assets_df = assets_df[
                    ~(
                        (assets_df.index.isin(exclude_dates))
                        & (assets_df["asset"] == exclude_asset)
                    )
                ]
            return assets_df
        ...
        for asset in assets:
            if asset in common_asset_names:
                self.common_assets = _remove_assets(
                    assets_df=self.common_assets,
                    exclude_asset=asset,
                    exclude_dates=assets[asset],
                )
            elif asset in reserve_asset_names:
                self.reserve_assets = _remove_assets(
                    assets_df=self.reserve_assets,
                    exclude_asset=asset,
                    exclude_dates=assets[asset],
                )
            else:
    ...
    def get_authorized_assets_for_dt(
        self, dt: str or datetime.datetime, assets_type: str = "common"
    ):
        """
