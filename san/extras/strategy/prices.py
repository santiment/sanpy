...
class Prices:
    ...
    def __init__(
        ...
        self.prices = pd.DataFrame(
            columns=["dt", "asset", "price", "price_change"]
        ).set_index("dt")
        self.fill_gaps_by_default = fill_gaps_by_default
        self.fill_gaps_function = fill_gaps_function

    def fill_prices_gaps(self):
        price_df = resample_dataframe(
            self.prices, self._granularity, "price", "asset", self.fill_gaps_function
        )
        self.set(price_df)
    ...
    def set(self, price_df: pd.DataFrame):
        ...
        if self.fill_gaps_by_default:
            price_df = resample_dataframe(
                price_df, self._granularity, "price", "asset", self.fill_gaps_function
            )
        ...
        price_df["price_change"] = (
            price_df.groupby("asset")
            .transform(lambda x: 1 + pd.DataFrame.pct_change(x))
            .fillna(1)
        )
