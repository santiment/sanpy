...
class Strategy:
    ...
    def add_periodic_rebalance(
        self, cron_expr: str, skip_rebalance_on_init: bool = True
    ):
        """
    ...
    def set_rebalance_proportion(self, proportions_df: pd.DataFrame):
        ...
        self.asset_shares = (
            self.asset_shares.groupby(["dt", "asset"])
            .last()
            .reset_index("asset")
            .sort_index()
        )

    def compute_asset_shares_for_dt(
        self, dt: str or datetime.datetime, assets: list = []
    ):
        """
        ...
        if dt not in self.asset_shares.index:
            logging.warning(
                f"Assets shares are missing for {dt}. The shares will be set to 1."
            )
            # TODO: keep same rebalance proportions
            self.set_rebalance_proportion(
                pd.DataFrame({"dt": dt, "asset": assets, "value": 1.0}).set_index("dt")
            )
    ...
    def set_default_rebalance_proportion(
        self, end_dt: datetime.datetime or str or None
    ):
        """
        ...
        df = pd.DataFrame(
            list(
                itertools.product(
                    pd.date_range(
                        self._start_dt, end_dt, freq=self._granularity
                    ),  # dates
    ...
    def generate_trade(
        self, share: float, asset_from: str, asset_to: str, fee=None, metadata: str = ""
    ):
        """
    ...
    def _init_strategy(self):
        ...
        assert (
            len(self.assets.get_names("all")) > 0
        ), "Add at least one asset to the portfolio!"

        if not self.init_asset:
            assert (
                len(self.assets.get_names("r")) > 0
            ), "Either init_asset or reserve_assets must be set up!"
    ...
    def build_portfolio(
        ...
        def _recompute_asset_shares(dt, prev_dt):
            ...
            df["share"] = (
                df["share"] * df["price_change"]
            )  # recompute share (so share column contains new shares)
            df["share"] = df["share"] / df["share"].sum()
            df = df[["dt", "asset", "share"]].set_index("dt")
            if abs(df["share"].sum() - 1) > self.accuracy:
                logging.warning(
                    f'The asset shares sum on {dt} is equal to {df["share"].sum()}'
        ...
        def _get_signals(dt):
            ...
            if hasattr(self, "cron") and dt >= self.cron.get_current(datetime.datetime):
                self.signals.add(
                    "r", pd.DataFrame({"dt": [dt]}), signal_name="cron_rebalance"
                )
                self.cron.get_next()
            ...
            if self.add_asset_once_authorized and dt != self._start_dt:
                ...
                assets_to_add_signals_to = (
                    ...
                    - set(
                        self.signals.get_signals_on_dt_asset_names_only(
                            dt=dt, signal_type="buy"
                        )
                    )
                )
            ...
            return {
                ...
                "buy_signals": self.signals.get_signals_on_dt(
                    dt, signal_type="b", assets=authorized_assets
                ),
        ...
        def _execute_trades(dt, trades):
            ...
            current_portfolio = {
                item["asset"]: item["share"] for i, item in current_portfolio.iterrows()
            }

            for trade in trades:
                if trade["from"] in current_portfolio and current_portfolio[
                    trade["from"]
                ] >= (trade["share"] - self.accuracy):
                    if (
                        abs(trade["share"] - current_portfolio[trade["from"]])
                        < self.accuracy
                    ):
                    ...
                    logging.error(
                        f"Trade can not be performed on {dt}: {trade}."
            ...
            if abs(sum(current_portfolio.values()) - 1) > self.accuracy:
                logging.warning(
                    f"Portfolio scructure sum is {sum(current_portfolio.values())}"
            ...
            result_df = pd.DataFrame(
                {
                    "dt": [dt] * len(current_portfolio),
                    "asset": list(current_portfolio.keys()),
                    "share": list(current_portfolio.values()),
                }
            )
        ...
        for dt in pd.date_range(start_dt, end_dt, freq=self._granularity):
            prev_dt = (
                ...
                else dt - parse_str_to_timedelta(self._granularity)
            )

            if dt in self.portfolio.index and dt != self._start_dt:
                logging.warning(
                    f"Portfolio for {dt} exists already. Skipping {dt}."
