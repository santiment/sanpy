...
class Signals:
    ...
    _default_signals_df = pd.DataFrame(
        columns=["dt", "signal", "asset", "trade_percantage", "decision_delay"]
    )
    ...
    def add(
        ...
        def _update_signals(
            signals: pd.DataFrame, df: pd.DataFrame, signal_name: str or None = None
        ):
            if not signal_name:
                ...
                while signal_name in signals["signal"].unique():
                    signal_name = (
                        signal_name.split("_")[0]
                        + "_"
                        + str(int(signal_name.split("_")[1]) + 1)
                    )
        ...
        if signal_type.lower() in ("buy", "b"):
            ...
            self.rebalance_signals = _update_signals(
                self.rebalance_signals, df, signal_name
            )
        else:
            logging.error(
                _shared_error_msg.format(signal_type)
            )  # Raise error instead of logging.error?
    ...
    def remove(self, signal_type: str, signal_name: str or None = None):
        ...
        def _update_signals(signals: pd.DataFrame, signal_name: str or None):
            if signal_name:
                logging.info(
                    f'Deleting {len(signals[signals["signal"] == signal_name])} signals named {signal_name}'
                )
                ...
                return signals.drop(
                    index=signals.index
                )  # delete all signals of a given type
        ...
        if signal_type.lower() in ("buy", "b"):
            ...
            self.rebalance_signals = _update_signals(
                self.rebalance_signals, signal_name
            )
        else:
    ...
    def get_signals_on_dt(self, dt: str, signal_type: str, assets: list = []):
        ...
        def _get_signals_on_dt_or_empty(signals_df, dt, assets):
            if dt in signals_df.index:
                if assets:
                    return signals_df[
                        (signals_df["asset"].isin(assets)) & (signals_df.index == dt)
                    ]
    ...
    def get_signals_on_dt_asset_names_only(
        self, dt: str, signal_type: str, assets: list = []
    ):
        """
