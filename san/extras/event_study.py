...
def get_close_price(data, sid, current_date, day_number, interval):
    ...
    # Find the close price day_number away from the current_date
        ...
        date_index = (
            total_date_index_length - 1
            if date_index >= total_date_index_length
            else date_index
        )
...
def plot_cumulative_abnormal_returns(returns, abnormal_returns, x_ticks, interval_text):
    ...
    abnormal_returns.plot(
        xticks=x_ticks, label="Abnormal Average Cumulative Returns", color=COLOR_2
    )
...
def plot_abnormal_cumulative_return_with_errors(
    abnormal_volatility, abnormal_returns, events
):
    """
...
def get_price_history(data, date, beta_window, sid, benchmark):
    ...
    if not beta_window:
        ...
        histotical_prices = data.iloc[history_index_start:history_index][
            [sid, benchmark]
        ]
    else:
        history_index = data.index.searchsorted(date)
...
def compute_return_matrix(
    ...
    for date, row in ev_data.iterrows():
        ...
        benchmark_return = get_returns(
            data, starting_point, benchmark, date, day_num, interval
        )
        ...
        beta = calc_beta(
            sid, benchmark, get_price_history(data, date, beta_window, sid, benchmark)
        )
        if beta is None:
            continue
...
def compute_averages(
    ...
    for day_num in day_numbers:
        ...
        all_benchmark_returns[day_num] = np.average(
            pd.Series(benchmark_returns).dropna()
        )
...
def clean_data(data, events, starting_point):
    ...
    for date, row in events_df.iterrows():
        ...
        if (
            event_window.min()[0] == 0
            or len(event_window) == 0
            or True in pd.isnull(list(event_window[sid]))
        ):
            events_df.iloc[id, -1] = 1
        id = id + 1
...
def plotting_events(
    ...
    plot_cumulative_return_with_errors(
        returns=all_returns, std_devs=all_std_devs, events=events
    )
...
def compute_beta_alpha(data, ev_data, starting_point, benchmark):
    ...
    for date, row in betas_df.iterrows():
        ...
        if sid == "ethereum" and benchmark == "ethereum":
            ...
            benchmark = "ethereum"
        coeff = calc_beta_testing(
            sid,
            benchmark,
            get_price_history(data, date, starting_point, sid, benchmark),
        )
        if coeff:
            beta, alpha = coeff
...
def ab_returns_matrix(ab_returns, betas_df, starting_point):
    ...
    for number, dta in betas_df.iterrows():
        ...
        my_sample = abnormal_returns_df.loc[
            (index - starting_point) : (index + starting_point), sid
...
def plot_CI(tstats, pvalues, CI):
    ...
    ax[0].axhline(
        ...
        color=COLOR_4,
        alpha=0.9,
        label="Significance Line (" + str(round(c, 2)) + ")",
    )
    ...
    ax[1].axhline(
        ...
        color=COLOR_4,
        alpha=0.9,
        label="Significance Line(" + str(round((1 - CI) / 2, 2)) + ")",
    )
    ax[1].legend()
...
def get_log_returns(data):
    ...
    returns_array = np.log(
        arithmetic_returns,
        out=np.zeros_like(arithmetic_returns),
        where=(arithmetic_returns != 0),
    )
