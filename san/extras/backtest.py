...
class Backtest:
    ...
    def add_trades(self, trades_df: pd.DataFrame, replace: bool = False):
        ...
        if replace:
            self.trades_log = self.trades_log[
                ~self.trades_log.index.isin(list(df.index))
            ]
        else:
    ...
    def get_available_portfolio_dts(self, df, start_dt, end_dt):
        ...
        if end_dt:
            dt2 = (
                end_dt
                if end_dt and type(end_dt) == datetime.datetime
                else str_to_ts(end_dt)
            )
    ...
    def build_net_returns(
        ...
        for dt in dts[1:]:
            ...
            dt_prices = dt_prices[
                dt_prices["asset"].isin(list(dt_portfolio["asset"].unique()))
            ]

            dt_portfolio = {
                el["asset"]: el["share"] for i, el in dt_portfolio.iterrows()
            }
            dt_prices = {
                el["asset"]: el["price_change"] for i, el in dt_prices.iterrows()
            }

            if abs(sum(dt_portfolio.values()) - 1) > self.accuracy:
                logging.warning(
                    f"Portfolio asset shares sum is different from 1 on {dt} "
                )
    ...
    def build_portfolio_price(
        ...
        if rebuild:
            self.portfolio_price = self.portfolio_price[
                self.portfolio_price.index <= start_dt
            ]
        ...
        for dt in dts[1:]:
            if dt in self.portfolio_price.index:
                logging.info(
                    f"The portfolio price data already contains values for {dt}"
                )
                continue
            ...
            if prev_dt not in self.portfolio_price.index:
                ...
                return
            dt_portfolio_price = float(
                self.portfolio_price.loc[prev_dt]["value"]
        ...
        self.portfolio_price["returns"] = 1 + self.portfolio_price[
            "value"
        ].pct_change().fillna(0)
