...
def get_labels_df(prices, signals, days, pt_sl, min_ret, lookback):
    ...
    vertical_barriers = ml.labeling.add_vertical_barrier(
        t_events=signals, close=prices, num_days=days
    )
...
def evaluate(
    prices, signals, pt_sl=[1, 2], min_ret=0.005, num_days=5, lookback=50, interval="1d"
):
    labels = pd.DataFrame()
...
def plot_rectangle(
    ax1, project_prices, labels, color, side, num_days, pt_sl, alpha=0.4, ls="--"
):
    for point in labels[labels["bin"] == side].index:
        xdata = [point, (point + pd.DateOffset(num_days))]
        ypoint_price_upper = project_prices[point] + (
            project_prices[point] * labels["trgt"][point] * pt_sl[0]
        )
        ypoint_price_lower = project_prices[point] - (
            project_prices[point] * labels["trgt"][point] * pt_sl[1]
        )
        ydata = [ypoint_price_upper, ypoint_price_upper]
        ax1.plot(xdata, ydata, color=color, alpha=alpha, ls=ls)
        ax1.plot(
            xdata,
            [ypoint_price_lower, ypoint_price_lower],
            color=color,
            alpha=alpha,
            ls=ls,
        )
